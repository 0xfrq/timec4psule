...
import { postCommentAPI } from "../api/feeed";
...
export default function Feed() {
  ...
  const videoElMapRef = useRef(new Map()); // id -> <video>
  ...
  // ===== helper: normalize year dari API (bisa number/string/obj {id,tahun})
  const normalizeYearValue = useCallback((p) => {
    const raw = p?.tahun;

    // kalau backend ngirim { id, tahun }
  ...
  // ===== helper: ambil uploaderId dari bentuk data apapun
  const getUploaderId = useCallback((p) => {
    ...
    // uploader: { id: 77 } atau uploader: { user: { id: 77 } }
  ...
  const fetchProfileName = useCallback(
    async (userId) => {
      ...
      try {
        ...
        if (uname) {
          setProfileNameById((prev) => ({ ...prev, [uid]: uname }));
        }
      } catch (e) {
  ...
  // ===== Fetch Topics/Comments (cache per video)
  ...
  // ===== 1) Load list (ONLY ONCE) - pakai tahunParam
  useEffect(() => {
    if (!apiBase) return;
    if (REQUIRE_TOKEN_FOR_LIST && !token) return;
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          listFetchedRef.current = false;
          throw new Error(`Error load video: ${res.status}`);
        }
        ...
        const mapped = items
          .filter((p) => String(p.media_type).toLowerCase() === "video")
          .map((p) => {
            ...
            return {
              ...
              title: p.description || `Video #${p.id}`,
  ...
  // ✅ jika videos bertambah (infinite scroll), fetch profile yang belum ada
  useEffect(() => {
  ...
  // ===== 2) Snap tracking (IntersectionObserver)
  useEffect(() => {
  ...
  // ===== 4) Fetch topics/comments once per video
  useEffect(() => {
    if (!activeVideoId) return;
  ...
  const submitComment = async () => {
    ...
    setCommentText("");
    try {
      await postCommentAPI(token, { post: activeVideoId, videoId: activeVideoId, text });

      const pid = String(activeVideoId);
      inFlightRef.current.comments.delete(pid);
      await fetchComments(pid);
    } catch (e) {
      console.error(e);
      fire({ icon: "error", title: "Gagal komentar", text: "Server error / endpoint POST comment belum sesuai." });
    }
  };
  ...
  const toggleLike = (id) => {
    ...
    setLikedByPost((prev) => ({ ...prev, [pid]: !prev[pid] }));
    setLikeCountByPost((prev) => {
      const cur = prev[pid] ?? 0;
      const currentlyLiked = !!likedByPost[pid];
      return { ...prev, [pid]: Math.max(0, cur + (currentlyLiked ? -1 : 1)) };
    });
  };
  ...
  const shareVideo = async (src) => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: "Video", text: "Check this out", url: urlToShare });
      } else {
  ...
  // ===== Infinite scroll (pakai tahunParam juga)
  const handleScroll = useCallback(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ fetch username uploader untuk batch baru
        const uniqUploaderIds = Array.from(new Set(newVideos.map((v) => v.uploaderId).filter(Boolean)));
        uniqUploaderIds.forEach((uid) => fetchProfileName(uid));
      } catch (e) {
        console.error(e);
      } finally {
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="hidden items-center gap-2 sm:flex">
              <div className="text-xs text-slate-600">
                ...
                <span className="text-slate-500">Video ID: {activeVideoId || "-"}</span>
              </div>
              <button
                onClick={logout}
        ...
        <aside className="hidden w-[340px] shrink-0 lg:block">
          <div className="flex h-full flex-col rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              ...
              <div className="text-xs text-slate-500">Video #{activeVideoId || "-"}</div>
            </div>
        ...
        <main className="flex min-w-0 flex-1 justify-center">
          <div className="w-full max-w-[520px]">
            <div
              ...
              className="h-full snap-y snap-mandatory scroll-smooth overflow-y-auto overscroll-contain rounded-2xl bg-white shadow-sm ring-1 ring-slate-200"
              style={{ WebkitOverflowScrolling: "touch" }}
            >
              <style>{`.tiktok-scroll::-webkit-scrollbar { width: 0px; height: 0px; }`}</style>

              <div className="tiktok-scroll">
                {!videos.length ? (
                  <div className="p-4 text-sm text-slate-600">
                    Tidak ada video. Cek response <b>/api/post/list/?tahun={tahunParam}</b>.
                  </div>
                ) : (
                  videos.map((v) => {
                      ...
                      >
                        <div className="relative h-[calc(100vh-4rem-3rem)] w-full bg-slate-900">
                          ...
                          <div className="absolute bottom-4 left-4 right-20 text-white">
                            {/* ✅ username asli dari /api/profiles/id?id=... */}
                            ...
                            <div className="mt-2 flex flex-wrap gap-2 text-xs text-white/90">
                              ...
                              <span className="rounded-full bg-white/15 px-3 py-1 backdrop-blur ring-1 ring-white/20">
                                <i className="ri-video-line mr-1" /> video
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="absolute right-3 top-1/2 -translate-y-1/2 space-y-3">
                            ...
                            <div className="text-center text-[11px] text-white/85">{likeCount || ""}</div>
